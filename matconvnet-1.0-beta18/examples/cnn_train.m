...
% opts.train = opts.train(1:1000);
% opts.val = opts.val(1:200);
% 
...
overload_rate = 1; % num. of training examples vs num. images
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    % compute the overload ratio batchsize here
    overload_rate = size(im, 4) / numel(batch); 
    if opts.prefetch
  ...
  if training
    if numGpus <= 1
      [net,res] = accumulate_gradients(opts, learningRate, batchSize*overload_rate, net, res) ;
    else
      if isempty(mmap)
      ...
      labBarrier() ;
      [net,res] = accumulate_gradients(opts, learningRate, batchSize*overload_rate, net, res, mmap) ;
    end
  end

  % collect and print learning statistics
  time = toc(start) ;
  stats = sum([stats,[0 ; error / overload_rate]],2); % works even when stats=[]
  stats(1) = time ;
...
for l=numel(net.layers):-1:1
  for j=1:numel(res(l).dzdw)
    ...
    if j == 3 && ...
       (strcmp(net.layers{l}.type, 'bnorm')  || ...
        (strcmp(net.layers{l}.type, 'custom') && strcmp(net.layers{l}.original_type, 'bnorm'))...
       )
